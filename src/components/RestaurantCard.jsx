...
const RestaurantCard = ({ restaurant }) => {
  const { id, name, cloudinaryImageId, cuisines, avgRating, areaName } =
    restaurant.info;
  return (
    <Link to={`/restaurant/${id}`} key={id}>
      <div
        data-testid="resCard"
        className="w-64 bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg hover:scale-105 transition-all duration-200"
      >
        <img
